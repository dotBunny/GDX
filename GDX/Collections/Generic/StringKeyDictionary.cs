...
namespace GDX.Collections.Generic
{
    ...
    public struct StringKeyDictionary<TValue>
    {
        ...
        /// <summary>
        /// Finds the bucket array index corresponding to a key.
        /// </summary>
        /// <param name="key">he key to find the bucket array index of.</param>
        /// <returns>The bucket array index of the entry.</returns>
        ...
        public int ModifyKeyUnchecked(int index, string newKey, out bool wasFirstEntryInChain)
        {
            if (newKey == null) throw new ArgumentNullException();

            ref var entryAtIndex = ref Entries[index];

            int hashCode = entryAtIndex.HashCode;
            int bucketIndex = hashCode % Buckets.Length;
            ...
            int nextUsedIndex = entryAtIndex.Next;

            wasFirstEntryInChain = indexOfKey == indexAtBucket;
            int indexToUse = wasFirstEntryInChain ? bucketIndex : previousIndex;
            ...
            int newHashCode = newKey.GetStableHashCode() & 0x7FFFFFFF;
            entryAtIndex.HashCode = newHashCode;
            entryAtIndex.Key = newKey;

            int newBucketIndex = hashCode % Buckets.Length;
            int newIndexAtBucket = Buckets[newBucketIndex];

            entryAtIndex.Next = newIndexAtBucket;
    ...
    [Serializable]
    public struct StringHashedStructKeyDictionary<TKey, TValue> where TKey : struct, IStringHashedStructKey, IEquatable<TKey>
    {
        public int[] Buckets;
        public StringHashedStructKeyEntry<TKey, TValue>[] Entries;
        public int FreeListHead;
        public int Count;
        ...
        /// </summary>
        /// <param name="minCapacity">The minimal initial capacity to reserve.</param>
        public StringHashedStructKeyDictionary(int minCapacity)
        {
            int primeCapacity = DictionaryPrimes.GetPrime(minCapacity);
            ...
            Entries = new StringHashedStructKeyEntry<TKey, TValue>[primeCapacity];
        ...
        /// <param name="key">The key to add.</param>
        /// <param name="value">The value to add.</param>
        public void AddWithExpandCheck(TKey key, TValue value)
        {
            if (key.stringToHash == null) throw new ArgumentNullException();
            ...
            int hashCode = key.stringToHash.GetStableHashCode() & 0x7FFFFFFF;
            int hashIndex = hashCode % Buckets.Length;
            int indexAtBucket = Buckets[hashIndex];
            ref StringHashedStructKeyEntry<TKey, TValue> entry = ref Entries[freeIndex];
        ...
        /// <param name="value">The value to add.</param>
        /// <returns>True if the entry was successfully created.</returns>
        public bool AddWithUniqueCheck(in TKey key, in TValue value)
        {
            if (key.stringToHash == null) throw new ArgumentNullException();
            ...
            while (nextKeyIndex > -1)
            {
                ref StringHashedStructKeyEntry<TKey, TValue> currEntry = ref Entries[nextKeyIndex];
                nextKeyIndex = currEntry.Next;
        ...
        /// <param name="value">The value to add.</param>
        /// <returns>True if the entry was successfully created.</returns>
        public bool AddSafe(in TKey key, in TValue value)
        {
            if (key.stringToHash == null) throw new ArgumentNullException();
            ...
            while (nextKeyIndex != -1)
            {
                StringHashedStructKeyEntry<TKey, TValue> currEntry = Entries[nextKeyIndex];
                nextKeyIndex = currEntry.Next;
                if (currEntry.Key.Equals(key))
                {
                    return false;
        ...
        /// <param name="key">The key to add.</param>
        /// <param name="value">The value to add.</param>
        public void AddUnchecked(in TKey key, in TValue value)
        {
            int dataIndex = FreeListHead;
            int hashCode = key.stringToHash.GetStableHashCode() & 0x7FFFFFFF;
            int bucketIndex = hashCode % Buckets.Length;
            ...
            ref StringHashedStructKeyEntry<TKey, TValue> entry = ref Entries[dataIndex];
        ...
        public void ExpandWhenFull()
        {
            ...
            StringHashedStructKeyEntry<TKey, TValue>[] newEntries = new StringHashedStructKeyEntry<TKey, TValue>[nextPrimeCapacity];
            Array.Copy(Entries, 0, newEntries, 0, oldCapacity);

            for (int i = 0; i < oldCapacity; i++)
            {
                ref StringHashedStructKeyEntry<TKey, TValue> entry = ref newEntries[i];
        ...
        public void Reserve(int capacityToReserve)
        {
            ...
            if (Count + capacityToReserve > oldCapacity)
            {
                ...
                for (int i = 0; i < oldCapacity; i++)
                {
                    ...
                    if (entry.Key.stringToHash != null)
                    {
                        int newBucketIndex = (entry.HashCode & 0x7FFFFFFF) % nextPrimeCapacity;
        ...
        public int BucketOf(in TKey key)
        {
            if (key.stringToHash == null) throw new ArgumentNullException();
        ...
        /// <param name="value">The new value of the entry.</param>
        /// <returns>True if the entry was found.</returns>
        public bool TryModifyValue(in TKey key, in TValue value)
        {
            if (key.stringToHash == null) throw new ArgumentNullException();
        ...
        public bool TryRemove(in TKey key)
        {
            ...
            while (indexOfKey != -1)
            {
                ref StringHashedStructKeyEntry<TKey, TValue> currEntry = ref Entries[indexOfKey];
        ...
        public void ModifyKeyUnchecked(int index, in TKey newKey)
        {
            if (newKey.stringToHash == null) throw new ArgumentNullException();
            ...
            int newHashCode = newKey.stringToHash.GetStableHashCode() & 0x7FFFFFFF;
            entryAtIndex.HashCode = newHashCode;
            entryAtIndex.Key = newKey;
        ...
        /// <param name="key">The key to remove.</param>
        /// <returns>True if the entry was found.</returns>
        public bool TryRemoveNoValueClear(in TKey key)
        {
            if (key.stringToHash == null) throw new ArgumentNullException();
        ...
        /// <param name="value">The value of the entry found.</param>
        /// <returns>True if the entry was found; false otherwise.</returns>
        public bool TryGetValue(in TKey key, ref TValue value)
        {
            if (key.stringToHash == null) throw new ArgumentNullException();
        ...
        /// <param name="entry">The entry returned by the iterator</param>
        /// <returns>Whether the iterator found an entry, finished iteration, or could not continue due to an invalid version.</returns>
        public IteratorState MoveNext(ref int iteratedIndexCount, int iteratorVersion, in int dictionaryVersion, ref StringHashedStructKeyEntry<TKey, TValue> entry)
        {
            entry = default;
            ...
            while (iteratedIndexCount < Entries.Length)
            {
                ref StringHashedStructKeyEntry<TKey, TValue> keyEntry = ref Entries[iteratedIndexCount];
                iteratedIndexCount++;

                if (keyEntry.Key.stringToHash != null)
                {
                    entry = keyEntry;
                    return IteratorState.FoundEntry;
                }
        ...
        /// <param name="entry">The entry returned by the iterator</param>
        /// <returns>Whether or not the iterator found an entry</returns>
        public bool MoveNext(ref int iteratedIndexCount, ref StringHashedStructKeyEntry<TKey, TValue> entry)
        {
            entry = default;
        ...
        public void Clear()
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ref StringHashedStructKeyEntry<TKey, TValue> entryAt = ref Entries[i];
                entryAt.Next = (1 << 31) | (i + 1);
                entryAt.Key = default;
                entryAt.Value = default;
                entryAt.HashCode = 0;
    ...
    public interface IStringHashedStructKey
    {
        public string stringToHash { get; }
    }

    [Serializable]
    public struct StringHashedStructKeyEntry<TKey, TValue> where TKey : IStringHashedStructKey, IEquatable<TKey>
    {
        public TKey Key;
        public TValue Value;
        public int Next;
