...
namespace GDX.Collections.Generic
{
    ...
    public struct StringKeyDictionary<TValue>
    {
        ...
        public int SwapKeyAtIndexUnchecked(int index, string newKey, out bool wasFirstEntryInChain)
        {
        ...
        /// <summary>
        /// Removes the entry at the selected index.
        /// </summary>
        /// <param name="key">The key to remove.</param>
        /// /// <param name="previousIndex">The index pointing to where the key was.</param>
        /// /// <param name="bucketWasPreviousIndex">Was the previous index the bucket index or an entry?</param>
        /// <returns>True if the entry was valid.</returns>
        public bool RemoveEntryAtIndexIfValid(int index, out int previousIndex, out bool bucketWasPreviousIndex)
        {
            ...
            int indexAtBucket = Buckets[bucketIndex];
            int indexOfKey = indexAtBucket;
            previousIndex = indexAtBucket;
            bucketWasPreviousIndex = indexOfKey != -1;
            ...
            if (foundIndex)
            {
                ref StringKeyEntry<TValue> currEntry = ref Entries[index];
                int nextUsedIndex = currEntry.Next;
                int nextFreeIndex = FreeListHead;
