...
namespace GDX.Editor
{
    ...
    public static class DataTableTracker
    {
        ...
        public static void RecordColumnDefinitionUndo(int tableTicket, int columnIdentifier, string actionDescription = null)
        {
            if (!k_TableTicketToTable.ContainsKey(tableTicket))
            {
                Debug.LogError($"A column definition undo event was recorded for {tableTicket}, but that ticket is invalid.");
                return;
            }
        ...
        public static void RecordRowDefinitionUndo(int tableTicket, int rowIdentifier, string actionDescription = null)
        {
            if (!k_TableTicketToTable.ContainsKey(tableTicket))
            {
                Debug.LogError($"A row definition undo event was recorded for {tableTicket}, but that ticket is invalid.");
                return;
            }
        ...
        static void OnUndoRedoEvent(in UndoRedoInfo undo)
        {
            ...
            if (int.TryParse(chunks[1], out int tableTicket))
            {
                ...
                if (!k_UndoRedoCallbackReceivers.ContainsKey(tableTicket)) return;
                ...
                if (string.CompareOrdinal(chunks[2], "Cell") == 0)
                {
                        ...
                        int.TryParse(chunks[4], out int columnIdentifier))
                    {
                        for (int i = 0; i < count; i++)
                        {
                            k_UndoRedoCallbackReceivers[tableTicket][i].OnUndoRedoCellValueChanged(rowIdentifier, columnIdentifier);
                        }
                    }
                }
                ...
                else if (string.CompareOrdinal(chunks[2], "Column") == 0)
                {
                    if (int.TryParse(chunks[3], out int columnIdentifier))
                    {
                        for (int i = 0; i < count; i++)
                        {
                            k_UndoRedoCallbackReceivers[tableTicket][i].OnUndoRedoColumnDefinitionChange(columnIdentifier);
                        }
                    }
                    else
                    {
