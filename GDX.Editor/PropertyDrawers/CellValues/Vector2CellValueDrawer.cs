...
{
    [CustomPropertyDrawer(typeof(Vector2CellValue))]
    public class Vector2CellValueDrawer : CellValueDrawerBase
    {
        Vector2CellValue m_CellValue;
        /// <inheritdoc />
        protected override void Init(SerializedProperty serializedProperty)
        {
            if (Reflection.TryGetFieldValue(serializedProperty.serializedObject.targetObject, typeof(Vector2CellValue),
                    serializedProperty.name, out Vector2CellValue cell))
            {
                m_CellValue = cell;
        ...
        protected override void CreateCellValue(TableBase table, int rowInternalIndex, int columnInternalIndex)
        {
            ...
            {
                m_CellValue = new Vector2CellValue(table, rowInternalIndex, columnInternalIndex);
            }
        }
        ...
        {
            return Vector2CellValue.GetSupportedType();
        }
    }
}